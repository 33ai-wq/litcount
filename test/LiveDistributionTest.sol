...
contract LiveDistributionTest is Test {
    zkLTC        public token;
    ...
    function setUp() public {
        ...
        // Create 21 test accounts
        for (uint256 i = 0; i < USER_COUNT; i++) {
            users[i] = address(uint160(i + 1)); // simple deterministic addresses
            // Give each account some initial ETH for gas (optional, but needed for transactions on testnet)
            // In a real test we would need to fund them, but for simulation we can assume they have ETH.
            // In Forge test environment, every account starts with a balance of 10000 ether.
        }
    }

    function test_LiveDistributionWith21Users() public {
